<template>
  <el-dialog size="large" :title="title" v-model="Visible" @close="close">
    <div class="importwarp">
      <div class="import">
        ...
        <div v-if="activeName=='json'" class="mockContent">
          <code-viewer :contents="'{}'" :options="{}" :ctype="'json'" :on-change="change"></code-viewer>
        </div>
        <div v-if="activeName=='java'" class="mockContent">
          <code-viewer :contents="''" :options="{}" :ctype="'java'" :on-change="javaChange"></code-viewer>
        </div>
    ...
    <div class="footer">
      <el-switch v-model="cover" on-color="#13ce66" on-text="追加" off-text="覆盖" off-color="#ff4949" :on-value="false" :off-value="true">
      </el-switch>
...
export default {
  ...
  methods: {
    ...
    importJson: function () {
      this.onImport(this.preData, this.cover)
      this.close()
    },
    onSubmit () {
    }
